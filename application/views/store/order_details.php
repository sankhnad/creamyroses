<?php
	...
	$paymentMode 	  = $orderObj[ 0 ]->payment_mode == '1'?'COD':'Online';
	$delivery_amount  = $orderObj[ 0 ]->delivery_price;
?>
  ...
  <table style="border:solid 1px #333333;" cellpadding="0" cellspacing="0">
    <tr>
      <td><table style="width:100%; max-width:1100px;" cellpadding="0" cellspacing="0">
          <tr>
            <td style="width:330px; padding:5px 10px;"><a href="<?=base_url();?>"<img style="width:50%;" src="<?=base_url();?>assets/store/images/logo.png"/> </td>
      ...
      <td style="border-top:#333 solid 1px;"><table width="100%" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:16px;">
          <tr>
            <td width="500px" style=" border-right:solid 1px #333333; padding:5px 10px;"><b>Order ID:
              ...
              <br />
              <b>Delivery Date:
              <?=date('jS M Y',strtotime($orderObj[0]->delivery_date));?>
              <br />
              <b>Delivery Time:
              <?=$orderObj[0]->delivery_option.'('.$orderObj[0]->delivery_time.')';?>
              <br />
              <b>Payment Mode:
              <?=$paymentMode;?>
              </b> </td>
    ...
    <tr>
      <td style="border-top:solid 1px #333333;"><table width="1100px" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:16px;">
          ...
          <tr>
            ...
            <td width="200px" style="text-align:right; border-bottom:#333 solid 1px; padding:5px 10px;"><?=$isEggLess?>
            </td>
            <td width="200px" style="text-align:right; border-bottom:#333 solid 1px; padding:5px 10px;"><?=round($total_price,2)?>
            </td>
          </tr>
          <?php }?>
		  <?php if($delivery_amount > 0){ 
		 			$subTotal = $subTotal - $delivery_amount;
		  ?>
		            <tr>

            <td colspan="7" style="padding-top:15px; text-align:right;"><b>Shipping Charges</b><br/>
            </td>
            <td style="text-align:right; padding:5px 10px;"><?=round($delivery_amount,2);?>
            </td>
          </tr>
		  <?php } ?>
          <tr>
        ...
        <?php if($orderObj[0]->coupon!=''){
					$discountVal = $orderObj[0]->coupon_price;
